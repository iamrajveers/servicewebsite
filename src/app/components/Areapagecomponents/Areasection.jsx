...
import { FaGlobeAmericas, FaMapMarkerAlt, FaChevronRight, FaDollarSign, FaStar } from 'react-icons/fa';
                        ...
                        <div className="relative w-full h-full">
                            {regions.flatMap((region, regionIndex) => (
                                region.cities.map((city, cityIndex) => (
                                    ...
                                    >
                                        <motion.div
                                            className={`mb-1 sm:mb-2 px-2 py-1 rounded-md text-xs font-bold whitespace-nowrap shadow-sm ${activeCity === city
                ...
                >
                    {regions.map((region, index) => (
                        <motion.div
                            key={index}
                            variants={item}
                            className={`bg-white rounded-xl sm:rounded-2xl shadow-lg sm:shadow-xl overflow-hidden border border-gray-200 transition-all duration-300 hover:shadow-xl sm:hover:shadow-2xl ${expandedIndices.includes(index) ? 'ring-2 ring-[#423F8D]' : ''} ${region.highlight ? 'border-t-4 border-t-[#423F8D]' : ''}`}
                        >
                            <div
                            ...
                            >
                                <div className="flex justify-between items-start">
                                    ...
                                    <FaChevronRight className={`w-4 h-4 sm:w-5 sm:h-5 text-gray-400 transition-transform ${expandedIndices.includes(index) ? 'rotate-90' : ''}`} />
                                </div>
                            </div>
                                            ...
                                            >
                                                {region.cities.map((city, cityIndex) => (
                                                    <motion.li
                                                        key={cityIndex}
                                                        variants={cityItem}
                                                        className={`flex items-center py-1 sm:py-2 px-2 sm:px-3 rounded-md sm:rounded-lg hover:bg-gray-50 transition-colors ${activeCity === city ? 'bg-gray-100 border-l-4 border-l-[#423F8D]' : ''}`}
                                                        onMouseEnter={() => setActiveCity(city)}
                                                        onMouseLeave={() => setActiveCity(null)}
                                                    >
